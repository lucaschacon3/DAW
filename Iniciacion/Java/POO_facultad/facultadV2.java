...
public class facultadV2 {
	...
	public static void changeTeacherDepartment() {
		...
		if ((pos<personas.length)&&(personas[pos]!=null)) {
			if(personas[pos] instanceof Profesor) {
				...
				profe.setDepartamento(nuevoDpto);
			}else {
	...
	public static Persona createPerson(int tipoPersona) {
		...
		if (tipoPersona==ESTUDIANTE) {
			...
			return new Estudiante(tipoPersona, nombre, apellido, dni, curso);
		}
		...
		if (tipoPersona==PROFESOR) {
			...
			String dpto=scanner.nextLine();
			return new Profesor(estadoCivil, anyo, despacho, nombre, apellido, dni, dpto);
		}
		if (tipoPersona==PERSONAL_SERVICIO) {
			System.out.println("Seccion");
			String seccion=scanner.nextLine();
			return new PersonalServicio(seccion, anyo, despacho, nombre, apellido, dni, seccion);
		}
		return null;
