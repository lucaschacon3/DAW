...
let idInterval= setInterval(reloj,1000) // si pones 0 te lo hace cada segundo empezando por el segundo 0

function reloj() {
	let idReloj=document.getElementById(`idReloj`)
	idReloj.innerHTML=`Hora: ${new Date().getHours()}:${ceros(new Date().getMinutes())}:${ceros(new Date().getSeconds())}`
	function ceros(hora) {
		if(hora<10){
			return (`0${hora}`)
		}
...
function gestionJugadores() {
	...
	let nombreJugadores=document.getElementById(`idNombreJugadores`)// guardas la etiqueta
	nombreJugadores.innerHTML=``// borras el contenido para que no se repitan al imprimir 

	jugadores.forEach((e)=>{
		nombreJugadores.innerHTML= `${nombreJugadores.innerHTML} ${e},`
	})
...
function gestionPuntuacion() {
	...
	crearElemento(`td`, `${puntos_j1}-${puntos_j2}`, document.getElementById(`idEstadisticas`))
	crearElemento(`td`, `${puntos_j1}-${puntos_j2}`, document.getElementById(`idEstadisticas`))
	crearElemento(`td`, `${puntos_j1}-${puntos_j2}`, document.getElementById(`idEstadisticas`))
}
